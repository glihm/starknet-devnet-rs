...
impl BackgroundAnvil {
    ...
    pub async fn get_balance_l1l2(
        ...
        address: Address,
        user: U256,
    ) -> Result<U256, TestError> {
        let l1l2_contract = abigen::L1L2Example::new(
            address,
            self.provider.clone(),
        );
        ...
        Ok(l1l2_contract
            ...
            .call()
            .await
            .map_err(|e| TestError::EthersError(
                format!("Error calling l1l2 contract on ethereum: {}", e)
            ))?)
    }
    ...
    pub async fn withdraw_l1l2(
        ...
        l1l2_contract
            ...
            .await
            .map_err(|e| TestError::EthersError(
                format!("tx for deposit l1l2 contract on ethereum failed: {}", e)
            ))?
            .await
            .map_err(|e| TestError::EthersError(
                format!("tx for deposit l1l2 contract on ethereum no receipt: {}", e)
            ))?;
...
async fn setup_ethereum_provider(rpc_url: &str) -> Result<(Arc<Provider<Http>>, Arc<SignerMiddleware<Provider<Http>, Wallet<SigningKey>>>), TestError> {
    ...
    let wallet: LocalWallet =
        ...
        .map_err(|e| TestError::EthersError(e.to_string()))?.with_chain_id(chain_id.as_u32());
